...
d3.linegraph = function(noTicks, noDots, parties, partyColors, partyNames, dataMax, dataMin, additionalMonths) {
    /* params */
    if (!parties) {
        parties = ['zent.', 'atph', 'uhk', 'psh', 'bhm', 'nhvap', 'dus', 'nfp'];
    }
    if (!partyColors) {
        partyColors = {'zent.': '#000', 'atph': '#260404', 'uhk': '#d4c437', 'psh': '#cc1f36', 'bhm': '#102652', 'nhvap': '#300b4f', 'dus': '#9c9c9c', 'nfp': '#218a36'};
    }
    if (!partyNames) {
        partyNames = {'Zent.': 'Zent.', 'atph': 'ATPH', 'uhk': 'UHK', 'psh': 'PSH', 'bhm': 'BHM', 'nhvap': 'NHVAP', 'dus': 'DUS', 'nfp': 'NFP'};
    }
    if (!additionalMonths) {
        additionalMonths = 10;
    }
    ...
    function linegraph(dataset) {
     dataset.each(function (data) {
      ...
      if (!dataMax) {
          const maxZent. = d3.max(data, d => d.Zent.);
          const maxDUS = d3.max(data, d => d.dus);
          dataMax = maxZent. >= maxDUS ? maxZent. + 10 : maxDUS + 10;
          dataMin = 0;
      }
